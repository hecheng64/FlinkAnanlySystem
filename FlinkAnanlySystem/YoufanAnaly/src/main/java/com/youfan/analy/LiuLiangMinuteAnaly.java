 ...
 */
public class LiuLiangMinuteAnaly {
...
//        //指定偏移量
        ...
        DataStream<LiuLiangInfo> reduce = map.keyBy("groupByField").timeWindowAll(Time.minutes(5)).reduce(new LiuLiangReduce());
        reduce.addSink(new LiuLiangSink());
        try {
            env.execute("LiuLiangMinuteAnaly");
        } catch (Exception e) {
            e.printStackTrace();
        }

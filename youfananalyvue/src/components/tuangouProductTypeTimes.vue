...
<script>
  ...
  export default {
    ...
    beforeCreate:function(){
      ...
      this.$http.post('http://127.0.0.1:9082/tuangouInfo/productTypeMinute',{"productTypeId": id},{emulateJSON: true}).then((response) => {
        this.option1 = {
          title: {
            text: '商品类别团购分钟级趋势'
          },
        ...
        this.$http.post('http://127.0.0.1:9082/tuangouInfo/productTypeHour',{"productTypeId": id},{emulateJSON: true}
        ).then((response) => {
          this.option2 = {
            title: {
              text: '商品类别团购小时级趋势'

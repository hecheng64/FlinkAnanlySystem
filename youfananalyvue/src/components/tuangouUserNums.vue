...
<script>
  ...
  export default {
    ...
    beforeCreate:function(){
      ...
      this.$http.post('http://127.0.0.1:9082/tuangouInfo/userNumsMinute').then((response) => {
        this.option1 = {
          title: {
            text: '团购成交订单用户数量分钟级点趋势'
          },
          yAxis: {
            title: {
              text: '用户数量'
        ...
        this.$http.post('http://127.0.0.1:9082/tuangouInfo/userNumsHour').then((response) => {
          this.option2 = {
            title: {
              text: '团购成交订单用户数量小时级趋势'
            },

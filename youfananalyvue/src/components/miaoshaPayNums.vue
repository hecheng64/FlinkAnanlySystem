...
<script>
  ...
  export default {
    ...
    beforeCreate:function(){
      ...
      this.$http.post('http://127.0.0.1:9082/miaoshaInfo/payNumsMinute').then((response) => {
        this.option1 = {
          title: {
            text: '秒杀成交订单支付类型分钟级趋势'
          },
        ...
        this.$http.post('http://127.0.0.1:9082/miaoshaInfo/payNumsHour').then((response) => {
          this.option2 = {
            title: {
              text: '秒杀成交订单支付类型小时级趋势'
            },
